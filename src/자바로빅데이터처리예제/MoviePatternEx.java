...
/*
 * CGV는 사이트에서 영화 댓글을 가져와서 분석하는 예제
 * 
 */
...
public class MoviePatternEx {
	public static void main(String[] args) {
		...
		//ㅎㅎ은 비정형데이터이다. //패턴으로 지운다.
		//영화 장르 초기화
		String[] feel = {"액션", "모험","어드벤처","스릴러","멜로","코미디","코딕","공포","드라마","판타지","경이","애니메이션","SF","뮤지컬"}; //분석하고 그래프하는 것이 빅데이터다.
		...
		//비정형 문자열을 정형 문자열로 변환
		String regex = "[A-EG-RT-Za-z0-9<>!~ㅋㅎ()?/.:']"; //sf를 제외한 값"[b0-9<>!~ㅋㅎ()?/.:']"
		
		//중접 for문
		for(int i=0; i<reply.length; i++) {
			reply[i] = reply[i].replaceAll(regex, "");   //""=>null값임
			...
			for(int j=0; j<feel.length; j++) {
				...
				result[j] = result[j] +count; //누적시키기 위해서 한 번더 같은 것으로 복사한다.
